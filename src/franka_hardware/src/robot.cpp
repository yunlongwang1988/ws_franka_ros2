...
void Robot::writeOnceJointVelocities(const std::array<double, 7>& velocities) {
  ...
  if (velocity_command_rate_limit_active_) {
    velocity_command.dq = franka::limitRate(
        robot_->getUpperJointVelocityLimits(current_state_.q_d),
        robot_->getLowerJointVelocityLimits(current_state_.q_d), franka::kMaxJointAcceleration,
...
void Robot::writeOnceJointPositions(const std::array<double, 7>& positions) {
  ...
  if (joint_position_command_rate_limit_active_) {
    position_command.q =
        franka::limitRate(robot_->getUpperJointVelocityLimits(current_state_.q_d),
                          robot_->getLowerJointVelocityLimits(current_state_.q_d),
